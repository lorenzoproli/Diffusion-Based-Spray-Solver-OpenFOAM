...
# -*- coding: utf-8 -*-
"""
Paper-style post-processing for OpenFOAM lagrangian droplet data,
following Zhang et al. (2026), adapted to parcel-based OpenFOAM.

Workflow reproduced from the paper:
1) merge 10 snapshots spaced by 0.5 ms in the stable atomization stage
2) build a 3D droplet-volume-fraction field on an equally spaced grid
3) log-transform and normalize the concentration field
4) project it onto front/top/left planes
5) extract effective areas using phi_thresh > 0.1, remove isolated
   small regions (<10 pixels), smooth with Gaussian sigma=3
6) extract sectional statistics at x = 30, 45, 60 mm downstream of the
   jet exit: parcel count, mean velocity, D10, D32 versus z, with a
   >25-particles/per-bin validity threshold
7) create overlay plots in paper style for U, D10, D32 and counts

IMPORTANT ADAPTATION:
OpenFOAM lagrangian data are PARCELS, not always individual droplets.
Therefore all concentration and diameter statistics are weighted by
nParticle to represent the physical droplet population.
"""
...
# Statistics planes at 30/45/60 mm downstream of jet exit
x_offsets = [0.030, 0.045, 0.060]   # [m]
slice_half_thickness = 5.0e-4       # [m]

# z-binning for sectional statistics
nz_stats = 13
z_bins = np.linspace(domain["z"][0], domain["z"][1], nz_stats + 1)
min_particles_per_bin = 25
...
OUTDIR = CASE_DIR / "paper_style_postproc_t0200_t0245"
...
def read_positions(path):
    """
    Robust reader for OpenFOAM vectorField files (positions, position0, U).
    """
...
def read_scalar(path):
    ...
    Reads only the entries inside the main scalar list.
    """
    txt = Path(path).read_text(encoding="latin-1", errors="ignore")
...
# =============================================================================
# SECTIONAL STATISTICS AT X = 30,45,60 mm
# =============================================================================
...
def compute_sectional_stats(pos, d, npart, U, x_plane, dx, z_bins, min_count):
    ...
    for i, (z0, z1) in enumerate(zip(z_bins[:-1], z_bins[1:])):
        ...
        out["count_droplets"][i] = np.sum(ni)

        if di.size < min_count:
            continue
...
# =============================================================================
# OVERLAY PLOTS IN PAPER STYLE
# =============================================================================

def plot_overlay_from_csv(outdir: Path, quantity_key: str, ylabel: str, out_png: str):
    labels = {
        "x_030mm_downstream": r"$\Delta x = 30$ mm",
        "x_045mm_downstream": r"$\Delta x = 45$ mm",
        "x_060mm_downstream": r"$\Delta x = 60$ mm",
    }

    styles = {
        "x_030mm_downstream": dict(marker="o", linewidth=2, color="tab:green"),
        "x_045mm_downstream": dict(marker="s", linewidth=2, color="tab:red"),
        "x_060mm_downstream": dict(marker="^", linewidth=2, color="tab:blue"),
    }

    files = [
        "x_030mm_downstream.csv",
        "x_045mm_downstream.csv",
        "x_060mm_downstream.csv",
    ]
    ...
    plt.grid(False)
    plt.tight_layout()
    plt.savefig(outdir / out_png, dpi=300)
    plt.close()
...
def project_field(phi_norm, mode="sum"):
    if mode == "sum":
        front = np.sum(phi_norm, axis=1)  # x-z
        top   = np.sum(phi_norm, axis=2)  # x-y
        left  = np.sum(phi_norm, axis=0)  # y-z
    elif mode == "max":
...
def main():
    ...
    # Sectional statistics at 30/45/60 mm downstream
    for xoff in x_offsets:
        x_plane = x_jet + xoff
        stats = compute_sectional_stats(
            pos, d, npart, U, x_plane, slice_half_thickness, z_bins, min_particles_per_bin
        )
        tag = f"x_{int(round(xoff*1e3)):03d}mm_downstream"
        save_sectional_stats(stats, OUTDIR / f"{tag}.csv")
        plot_sectional_stats(stats, tag, OUTDIR / tag)

    # Overlay plots in paper style
    plot_overlay_from_csv(
        OUTDIR,
        quantity_key="Umean_m_s",
        ylabel="U [m/s]",
        out_png="overlay_U.png"
    )
    ...
    plot_overlay_from_csv(
        ...
        quantity_key="D32_m",
        ylabel="D32 [µm]",
        out_png="overlay_D32.png"
    )

    plot_overlay_from_csv(
        OUTDIR,
        quantity_key="count_parcels",
        ylabel="Parcel count [-]",
        out_png="overlay_count_parcels.png"
    )
    ...
    plot_overlay_from_csv(
        ...
        out_png="overlay_count_droplets.png"
    )
    ...
    save_projection_plot(front, "x vs z", OUTDIR / "projection_front_xz.png", [x_mm[0], x_mm[1], z_mm[0], z_mm[1]])
    save_projection_plot(top,   "x vs y", OUTDIR / "projection_top_xy.png",   [x_mm[0], x_mm[1], y_mm[0], y_mm[1]])
    save_projection_plot(left,  "y vs z", OUTDIR / "projection_left_yz.png",  [y_mm[0], y_mm[1], z_mm[0], z_mm[1]])
    ...
    save_boundary_overlay(front, front_proc, "x vs z", OUTDIR / "boundary_front_xz.png", [x_mm[0], x_mm[1], z_mm[0], z_mm[1]])
    save_boundary_overlay(top,   top_proc,   "x vs y", OUTDIR / "boundary_top_xy.png",   [x_mm[0], x_mm[1], y_mm[0], y_mm[1]])
    save_boundary_overlay(left,  left_proc,  "y vs z", OUTDIR / "boundary_left_yz.png",  [y_mm[0], y_mm[1], z_mm[0], z_mm[1]])

    print(f"Used times: {used_times}")
